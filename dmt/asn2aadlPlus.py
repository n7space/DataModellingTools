...
def main():
    ...
    if not bAADLv2:
        #  until 2024, the v1 file was used by aadl2glueC, which parsed it back
        #  to get the type names. The problem is that this parsing of the AADL
        #  file was done with antlr2 and required python2. In the meantime,
        #  Python2 was abandoned and removed from Debian repositories. 
        #  while the AADLv1 file can still be generated here, it should not be
        #  further used anywhere. Instead a generation of python3 code (without
        #  any intermediate AADL file) is done when not targeting aadlv2
        #  the AADLv2 file is parsed by Ocarina, so this is not affected by
        #  the lack of Python2 support.
        with open(absPathOfAADLfile + '.py', 'w') as v1:
            v1.write('#!/usr/bin/env python3\n\n')
            v1.write('# File generated by asn2aadlPlus: DO NOT EDIT\n')
            v1.write('from dmt.commonPy import aadlAST as ast\n\n')
            v1.write('ast.g_signals = dict()\n\n')
            for asnTypename in sorted(list(asnParser.g_names.keys())):
                ...
                src_text = asnParser.g_names[asnTypename]._asnFilename
                type_name = asnTypename
                data_size = 0 if asnTypename not in messageSizes \
                        else messageSizes[asnTypename][1]
                v1.write(
                 f's = ast.Signal("{src_text}", "{type_name}", {data_size})\n')
                v1.write(f'ast.g_signals["{cleanName}"] = s\n')
                v1.write(f'ast.g_signals["DataView::{cleanName}"] = s\n')
