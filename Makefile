...
PY_SRC:=$(filter-out dmt/A_mappers/scade6_A_mapper.py dmt/B_mappers/antlr.main.py dmt/A_mappers/Stubs.py dmt/B_mappers/micropython_async_B_mapper.py dmt/commonPy/commonSMP2.py, ${PY_SRC})
...
install:	configure
	python3 -m pip uninstall --break-system-packages -y dmt || exit 0  # Uninstall if there, but don't abort if not installed
